...
var cart={'add':function(product_id,quantity){var $product=$('.product-id_'+product_id).parent();$.ajax({url:'index.php?route=checkout/cart/add',type:'post',data:'product_id='+product_id+'&quantity='+(typeof(quantity)!='undefined'&&quantity!=0?quantity:1),dataType:'json',beforeSend:function(){$product.find('.wait').remove().end().removeClass('tb_product_loaded tb_add_to_cart_loaded').addClass('tb_product_loading tb_add_to_cart_loading').find('.tb_button_add_to_cart').append('<span class="wait"></span>');},success:function(json){$('.alert, .text-danger').remove();if(json['redirect']){location=json['redirect'];}else
if(json['error']){for(var first in json.error)break;displayNotice('product','failure',product_id,json.error[first]);}else
if(json['success']){setTimeout(function(){$product.removeClass('tb_product_loading tb_add_to_cart_loading').find('.tb_button_add_to_cart .wait').remove().end().addClass('tb_product_loaded tb_add_to_cart_loaded');return;setTimeout(function(){$product.removeClass('tb_product_loaded tb_add_to_cart_loaded')},5000);},500);$.get('index.php?route=common/cart/info',function(result){var $items=$(result).find('.content').clone(),$title=$(result).find('.heading').clone();$('.tb_wt_header_cart_menu_system .heading').replaceWith($title);$('.tb_wt_header_cart_menu_system .content').replaceWith($items);tbApp.triggerResizeCallbacks();});displayNotice('product','success',product_id,json['success']);}}});},'update':function(key,quantity){$.ajax({url:'index.php?route=checkout/cart/edit',type:'post',data:'key='+key+'&quantity='+(typeof(quantity)!='undefined'?quantity:1),dataType:'json',beforeSend:function(){$('#cart > button').button('loading');},success:function(json){$('#cart > button').button('reset');if(tbApp['/tb/route']=='checkout/cart'||tbApp['/tb/route']=='checkout/checkout'){location='index.php?route=checkout/cart';}else{$('.tb_wt_header_cart_menu_system').load('index.php?route=common/cart/info');}}});},'remove':function(key){$.ajax({url:'index.php?route=checkout/cart/remove',type:'post',data:'key='+key,dataType:'json',beforeSend:function(){$('#cart > button').button('loading');},success:function(json){$('#cart > button').button('reset');if(tbApp['/tb/route']=='checkout/cart'||tbApp['/tb/route']=='checkout/checkout'){location=tbApp['/tb/url/shopping_cart'];}else{$.get('index.php?route=common/cart/info',function(result){var $items=$(result).find('.content').clone(),$title=$(result).find('.heading').clone();$('.tb_wt_header_cart_menu_system .heading').replaceWith($title);$('.tb_wt_header_cart_menu_system .content').replaceWith($items);tbApp.triggerResizeCallbacks();});}}});}};var voucher={'add':function(){},'remove':function(key){$.ajax({url:'index.php?route=checkout/cart/remove',type:'post',data:'key='+key,dataType:'json',beforeSend:function(){$('#cart > button').button('loading');},complete:function(){$('#cart > button').button('reset');},success:function(json){$('#cart-total').html(json['total']);if(tbApp['/tb/route']=='checkout/cart'||tbApp['/tb/route']=='checkout/checkout'){location='index.php?route=checkout/cart';}else{$('.tb_wt_header_cart_menu_system').load('index.php?route=common/cart/info');}}});}};var wishlist={'add':function(product_id){var $product=$('.product-id_'+product_id).parent();$.ajax({url:'index.php?route=account/wishlist/add',type:'post',data:'product_id='+product_id,dataType:'json',beforeSend:function(){$product.find('.wait').remove().end().removeClass('tb_product_loaded tb_wishlist_loaded').addClass('tb_product_loading tb_wishlist_loading').find('.tb_button_wishlist').append('<span class="wait"></span>');},success:function(json){$('.alert').remove();if(json['success']){displayNotice('wishlist','success',product_id,json['success']);}
if(json['info']){displayNotice('wishlist','failure',product_id,json['info']);}
setTimeout(function(){$product.removeClass('tb_product_loading tb_wishlist_loading').find('.tb_button_wishlist .wait').remove().end().addClass('tb_product_loaded tb_wishlist_loaded');return;setTimeout(function(){$product.removeClass('tb_product_loaded tb_wishlist_loaded')},5000);},500);$('.wishlist-total').html(json['total']);Array.prototype.forEach.call(document.querySelectorAll('a.wishlist_total, li.wishlist_total > a > .tb_text'),function(el){var number=json['total'].replace(/[^0-9]/g,'');$(el).find('.tb_items').remove();$(el).append('<span class="tb_items">'+number+'</span>');});}});},'remove':function(){}};var compare={'add':function(product_id){$.ajax({url:'index.php?route=product/compare/add',type:'post',data:'product_id='+product_id,dataType:'json',success:function(json){$('.alert').remove();if(json['success']){displayNotice('compare','success',product_id,json['success']);$('#compare-total').html(json['total']);Array.prototype.forEach.call(document.querySelectorAll('a.tb_compare_total, li.tb_compare_total > a > .tb_text'),function(el){var number=json['total'].replace(/[^0-9]/g,'');$(el).find('.tb_items').remove();$(el).append('<span class="tb_items">'+number+'</span>');});}}});},'remove':function(){}};function moduleSearch($element){var filter_name=$element.parent().find('input[name=search]').val();if(filter_name){location=$('base').attr('href')+'index.php?route=product/search&search='+encodeURIComponent(filter_name);}}
...
this.show=function(){var pos=$(this).position();$(this).siblings('ul.dropdown-menu').css({display:'block',top:pos.top+$(this).outerHeight(),left:pos.left});$(this).siblings('ul.dropdown-menu').show();};this.hide=function(){$(this).siblings('ul.dropdown-menu').hide();};this.request=function(){clearTimeout(this.timer);this.timer=setTimeout(function(object){object.source($(object).val(),$.proxy(object.response,object));},200,this);};this.response=function(json){html='';if(json.length){for(i=0;i<json.length;i++){this.items[json[i]['value']]=json[i];}
for(i=0;i<json.length;i++){if(!json[i]['category']){html+='<li data-value="'+json[i]['value']+'"><a href="#">'+json[i]['label']+'</a></li>';}}
var category=new Array();for(i=0;i<json.length;i++){if(json[i]['category']){if(!category[json[i]['category']]){category[json[i]['category']]=new Array();category[json[i]['category']]['name']=json[i]['category'];category[json[i]['category']]['item']=new Array();}
category[json[i]['category']]['item'].push(json[i]);}}
for(i in category){html+='<li class="dropdown-header">'+category[i]['name']+'</li>';for(j=0;j<category[i]['item'].length;j++){html+='<li data-value="'+category[i]['item'][j]['value']+'"><a href="#">&nbsp;&nbsp;&nbsp;'+category[i]['item'][j]['label']+'</a></li>';}}}
if(html){this.show();}else{this.hide();}
$(this).siblings('ul.dropdown-menu').html(html);};$(this).after('<ul class="autocomplete-menu dropdown-menu"></ul>');$(this).siblings('ul.dropdown-menu').delegate('a','click',$.proxy(this.click,this));});}})(window.jQuery);tbApp.onScriptLoaded(function(){tbApp.windowWidth=window.innerWidth;tbUtils.onSizeChange(function(){tbApp.windowWidth=window.innerWidth;},false,false,'windowWidth');includeSvgResource("catalog/view/theme/"+tbApp["/tb/basename"]+"/image/icons.svg",2);});var tb_navigation_hovered=false;function displayNotice(context,type,product_id,message){var stack=Number(tbApp['/tb/msg_stack']);var thumb;if(!stack){$.noty.closeAll();}
...
var $cont=$('#header'),$wrap=$('#wrapper'),$temp;var height=$cont.outerHeight(true),offset=$cont.offset(),old_classes=$cont.attr('class'),classes=layout!='full_fixed'?'tb_sticky_width_'+layout:'tb_sticky_width_full tb_content_fixed';tbApp.onWindowLoaded(function(){$(window).scroll(function(){if(window.pageYOffset>height+offset.top-20&&tbApp.windowWidth>768&&!tb_navigation_hovered){if($wrap.find('> .tbSticky').length==0){function clone_widget($widget){tbApp.trigger('beforeCloneWidget',[$widget]);var $clonedWidget=$widget.clone(true).css({'padding-left':0,'padding-right':0}).removeClass('display-block').addClass('display-inline-block').find("script").remove().end();$temp.find('.col-md-12').append($clonedWidget);}
if(style=='minimal'&&$('#sticky_header').length==0){$temp=$('<div id="sticky_header" class="tb_area_header '+classes+' tbSticky" style="padding: '+padding+'"><div class="row tbStickyRow"><div class="col col-xs-12 col-md-12 col-valign-middle"></div></div></div>').prependTo($wrap);$cont.find('.tbStickyShow:not([class*="tbStickyPosition"])').each(function(){clone_widget($(this));});for(var i=1;i<=8;i++){$cont.find('.tbStickyPosition-'+i).each(function(){clone_widget($(this));});}
$temp.find('.tb_hovered').removeClass('tb_hovered');$temp.find('[id]:not(.tb_wt):not(#cart):not(#search):not(#site_logo)').each(function(){$(this).attr('id',$(this).attr('id')+'_cloned');});$temp.find('[data-target]').each(function(){$(this).data('target',$(this).data('target')+'_cloned');});dropdown_menu('#sticky_header .dropdown');$temp.find('.tb_tabbed_menu').each(function(){tabbed_menu($(this).attr('id'));});}
if(style=='default'){$cont.addClass(classes+' tbSticky').css('padding',padding);$('body').css('padding-top',($cont.hasClass('tb_header_overlay')?0:height));}}
else{$('#sticky_header').show();}}
else{if(style=='minimal'){$('#sticky_header').hide();}
if(style=='default'){$cont.attr('class',old_classes);}
$cont.css('height','').css('padding','');$('body').css('padding-top','');$cont.css('height','');}});$(window).trigger('scroll');});}
function responsive_header(){var $temp;$('#header').on('click','.tbToggleButtons span',function(){if($(this).hasClass('tbToggleCart')){$temp=$('<div class="tbMobileMenu tbMobileCartMenu"><div class="row"><div class="col col-xs-12 col-sm-12 col-valign-top"></div></div></div>').appendTo('#wrapper');$('#header').find('.tb_wt_header_cart_menu_system').each(function(){var $clonedMenu=$(this).clone(true).find("script").remove().end();$temp.find('.col-sm-12').append($clonedMenu);$('html').addClass('tbCartMenu');element_query('.tbMobileCartMenu .cart-info','500,300,0');});}else{$temp=$('<div class="tbMobileMenu"><div class="row"><div class="col col-xs-12 col-sm-12 col-valign-top"></div></div></div>').appendTo('#wrapper');$('#header').find('.tbMobileMenuShow:not([class*="tbMobilePosition"])').each(function(){var $clonedMenu=$(this).clone(true).find("script").remove().end();$temp.find('.col-sm-12').append($clonedMenu);});for(var i=1;i<=8;i++){$('#header').find('.tbMobileMenuShow.tbMobilePosition-'+i).each(function(){var $clonedMenu=$(this).clone(true).find("script").remove().end();$temp.find('.col-sm-12').append($clonedMenu);});}
...
$(container).find('.tb_listing').after('<div class="tb_slider_controls">'+'  <a class="tb_prev" href="javascript:;" title="'+tbApp._t('text_previous')+'">'+'    <svg><use xlink:href="'+window.location.href+'#chevron_thin" /></svg>'+'  </a>'+'  <a class="tb_next" href="javascript:;" title="'+tbApp._t('text_next')+'">'+'    <svg><use xlink:href="'+window.location.href+'#chevron_thin" /></svg>'+'  </a>'+'</div>');SwiperObj=new Swiper(container+' .swiper-container',{setWrapperSize:true,slidesPerView:slides_per_view,slidesPerGroup:slide_step,speed:speed,autoplay:autoplay,loop:loop,loopAdditionalSlides:slides_per_view,pagination:pagination,paginationClickable:true,slideActiveClass:'',slideVisibleClass:'',slideNextClass:'',slidePrevClass:'',bulletActiveClass:'tb_active',onSwiperCreated:function(swiper){buttons();$(container).find('.swiper-container').addClass('tb_slider_init');},onSlideChangeEnd:function(swiper){buttons();},onTouchEnd:function(swiper){buttons();}});$(container).on('click','.tb_prev, .tb_next',function(){$(container).addClass('tbSliderInteracted');});$(container).on('mouseover',function(){SwiperObj.stopAutoplay();});$(container).on('mouseout',function(){if(!$(this).is('.tbSliderInteracted')){SwiperObj.startAutoplay();}});swiperDeferred.resolve(SwiperObj);}
...
else{var $container=$(container).is('.has_slider:not(.no_title)')?$(container):$(container).closest('.tb_wt_group'),lang_dir=$('html').first().attr('dir'),mod_x=lang_dir=='ltr'?parseInt($heading.css('padding-right'))
+parseInt($container.css('padding-right'))
+parseInt($heading.css('margin-right')):parseInt($heading.css('padding-left'))
+parseInt($container.css('padding-left'))
+parseInt($heading.css('margin-left')),mod_y=($heading.height()-$nav.outerHeight())/2+parseInt($heading.css('padding-top'))+parseInt($container.css('padding-top'));mod_x+=mod_x>=15?-5:0;$nav.css('top',mod_y);$nav.css(lang_dir=='ltr'?'right':'left',mod_x-4);}}
...
noty({text:'<h3>'+tbApp._t('text_cookie_policy_title')+'</h3><p>'+policy_text+'</p>',layout:tbApp['/tb/msg_position'],closeOnSelfClick:false,modal:false,buttons:[{type:'btn',text:tbApp._t('text_cookie_policy_button'),click:function(){$.cookie('agreed_with_cookie_policy','1');$.noty.closeAll();}}],closeButton:false,timeout:false,animateOpen:{opacity:'toggle'},animateClose:{opacity:'toggle'},close_speed:500,onClose:function(){$(document).unbind('touchmove.noty');}});}
function dropdown_menu(selector){if(selector===undefined){selector='.dropdown';}
Array.prototype.forEach.call(document.querySelectorAll(selector),function(el){hoverintent(el,function(){var tb_sticky_parent;if($(this).parent().is('.tb_grid_view')||$(this).parent().is('.tab-content')){return;}
$(this).addClass('tb_hovered');if($(this).closest('#header').length){tb_navigation_hovered=true;}
if($(this).closest('.tbSticky').length){tb_sticky_parent=true;}
var $nav=$(this).closest('.nav'),$menu=$(this),$dropdown=$(this).find('.dropdown-menu').first(),$relativeTo=$(this).closest('[class*="tb_area_"], .tbMobileMenu'),orientation=$nav.length&&$nav.hasClass('nav-horizontal')?'horizontal':'vertical';if(tb_sticky_parent&&$menu.parent('.nav').length&&($dropdown.outerHeight()+$dropdown.offset().top-$(window).scrollTop>window.innerHeight)){$dropdown.css('max-height',(window.innerHeight-110));$dropdown.css('overflow-y','auto');}
if(!$nav.length||!$menu.hasClass('tb_megamenu')){var menu_space=$dropdown.outerWidth()+$dropdown.offset().left;var margin_left=menu_space>tbApp.windowWidth?tbApp.windowWidth-menu_space-30:0;if(margin_left==0){return;}
$dropdown.css('margin-left',margin_left);}
if(!$nav.length||!$dropdown.length||!$menu.hasClass('tb_megamenu')){return;}
var menu_left,menu_top,menu_width,current_site_width=tbApp['/tb/maximum_width'];if(tbApp.windowWidth-60<=tbApp['/tb/maximum_width']){current_site_width=tbApp.windowWidth-60;}
...
if($menu.find('.tb_grid_view')){adjustItemSize('#'+$menu.attr('id')+' .tb_subcategories',$menu.data('responsive'));}
if($menu.is('.tb_tabbed_menu')){var $link=$dropdown.find('> .tb_tabs > .nav-tabs > li.active'),tab='#'+$link.find('> a').data('target');$dropdown.find('> .tb_tabs > .nav-tabs').css('min-width',$menu.width());if($(tab+' .tb_grid_view').length){adjustItemSize(tab+' .tb_subcategories',$link.data('responsive'));}}},function(){$(this).removeClass('tb_hovered').find('.dropdown-menu').first().css('margin-left','');tb_navigation_hovered=false;}).options({timeout:300});});}
function tabbed_menu(menu){var $nav=$(menu).closest('.nav');Array.prototype.forEach.call(document.getElementById(menu).querySelectorAll('.nav-tabs > li > a'),function(el){hoverintent(el,function(){var $menu=$('#'+menu),$link=$(el).parent(),tab='#'+$(el).data('target');$menu.find('.tab-content > *').removeClass('tb_opened');$menu.find('.nav-tabs > *').removeClass('active');$(this).parent().addClass('active');$(tab).addClass('tb_opened');$menu.toggleClass('tb_first_tab_selected',$menu.find('.nav-tabs > li:first-child').is('.active'));if($(tab+' .tb_grid_view').length){adjustItemSize(tab+' .tb_subcategories',$link.data('responsive'));}},function(){}).options({timeout:300});});}
...
var $uiButton=$.fn.button;$.fn.button=function(){if(arguments.length==1&&typeof arguments[0]=='string'){var $button=$(this);var position=$button.parent().hasClass('pull-right')?'before':'after';if(arguments[0]=='loading'){$button.attr('disabled',true);if(position=='before'){$button.prev('.fa-spin').remove();$button.before('<i class="fa fa-circle-o-notch fa-spin"></i>');}else{$button.next('.fa-spin').remove();$button.after('<i class="fa fa-circle-o-notch fa-spin"></i>');}}
if(arguments[0]=='reset'){$button.attr('disabled',false);if(position=='before'){setTimeout(function(){$button.prev('.fa-spin').fadeOut(300,function(){$(this).remove();});},500);}else{setTimeout(function(){$button.next('.fa-spin').fadeOut(300,function(){$(this).remove();});},500);}}}else{$uiButton.apply(this,arguments);return this;}};$('span.error, .text-danger').each(function(){$(this).closest('tr, .form-group').addClass('has-error');});$body.on('click','.alert, .success, .attention, .warning, .information',function(){$(this).fadeTo(200,0,function(){$(this).slideUp(300,function(){$(this).remove();});});});$('#currency, #form-currency').on('click','a[data-currency-code]',function(){changeCurrency($(this));});$('#language, #form-language').on('click','a[data-language-code]',function(){changeLanguage($(this));});$('#search_button').bind('click',function(){moduleSearch($(this));});$('#filter_keyword').bind('keydown',function(e){if(e.keyCode==13){var $element=$(this);tbApp.once('filterKeywordEnter',function(context){if(context.redirect){moduleSearch($element);}});tbApp.trigger('filterKeywordEnter',[{redirect:true}]);}});$body.on('click','.agree, .colorbox, .fancybox, .tb_popup',function(e){e.preventDefault();var $link=$(this);if($link.data('dialog')){$('#'+$link.data('dialog')).modal('show');}else{var dialog_id='tbModal-'+Math.floor(Math.random()*100)+1;html='<div id="'+dialog_id+'" class="modal fade">';html+='  <div class="modal-dialog">';html+='    <div class="modal-content">';html+='      <div class="modal-header">';html+='        <h2 class="modal-title">'+$link.text()+'</h2>';html+='        <a href="javascript:;" class="close" data-dismiss="modal"></a>';html+='      </div>';html+='      <div class="modal-body"></div>';html+='    </div>';html+='  </div>';html+='</div>';$('body').append(html);$('#'+dialog_id).modal('show').on('shown.bs.modal',function(e){if($link.data('dialog')){return;}
var jqxhr=$.ajax({url:$link.attr('href'),type:'get',dataType:'html',beforeSend:function(){$('#'+dialog_id).find('.modal-body').addClass('tb_loading tb_blocked').append('<span class="wait"></span>');}});setTimeout(function(){jqxhr.done(function(data){$('#'+dialog_id).find('.modal-body').html(data).removeClass('tb_loading tb_blocked');});},500);$link.data('dialog',dialog_id);});}});element_query('.responsive');element_query('.cart-info','500,300,0');element_query('#cart','500,300,0','.cart-info');element_query('.tb_listing_options','1000,650,350,0');element_query('.tb_wt_product_options_system','1000,300,0');element_query('.tb_wt_product_add_to_cart_system','400,0');element_query('.tb_system_search_box','550,0');if(tbUtils.is_touch){$body.on('click',function(e){$('.dropdown').removeClass('tb_hovered');$('.tb_toggle').removeClass('tb_active');}).on('click','.dropdown > a',function(e){if($(this).siblings('.dropdown-menu,ul')[0].getBoundingClientRect().width<1){$('.dropdown').removeClass('tb_hovered');$(this).parents('.dropdown').addClass('tb_hovered');$(this).parent().find('> .tb_toggle').addClass('tb_active');}
else{$(this).parent().removeClass('tb_hovered');$(this).parent().find('> .tb_toggle').removeClass('tb_active');window.location=$(this).attr('href');}
return false;});}
$body.on(tbUtils.is_touch?'touchstart MSPointerDown':'click','.dropdown > .tb_toggle',function(){if($(this).siblings('.dropdown-menu,ul')[0].getBoundingClientRect().width<1){$(this).parent().addClass('tb_hovered').find('> .tb_toggle').addClass('tb_active');}else{$(this).parent().removeClass('tb_hovered').find('> .tb_toggle').removeClass('tb_active');}
if($(this).parent().find('.tb_grid_view')){adjustItemSize('#'+$(this).parent().attr('id')+' .tb_subcategories',$(this).parent().data('responsive'));}
return false;});dropdown_menu();responsive_header();$('#wrapper').find('.tb_wt_cart > .nav').each(function(){var $nav=$(this),url=window.location.href;$nav.find('> .heading a').one('mouseenter',function(){$nav.addClass('active').load('index.php?route=common/cart/info .nav > *').on('mouseleave',function(){$(this).removeClass('active');});});});}
